...
// One iteration is actually 4 semaphore operations. We're only interested in
// direct contention between two processes. Thus, the program will utilize at
// most 2 cores.
void perform_sem_test(int num_iterations, sem_t* sem1, sem_t* sem2) {
	pid_t cid = fork();
	if(cid) {
...
void sem_test(int num_iterations) {
	...
	sem_init(sems, 1, 0);
	sem_init(sems + 1, 1, 0);
	perform_sem_test(num_iterations, sems, sems + 1);
	sem_destroy(sems);
	sem_destroy(sems + 1);
