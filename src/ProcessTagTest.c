...
/*
 * We're only interested in how quickly we can tag between with 2 processes.
 * Thus, this program will use at most 2 cores. This is analogous to game of tag
 * between two processes.
 */
void perform_process_tag_test(int num_iterations, sem_t* sem1, sem_t* sem2) {
	pid_t cid = fork();
	if(cid) {
...
void process_tag_test(int num_iterations) {
	...
	sem_init(sems, 1, 0);
	sem_init(sems + 1, 1, 0);
	perform_process_tag_test(num_iterations, sems, sems + 1);
	sem_destroy(sems);
	sem_destroy(sems + 1);
...
int main(int argc, char** argv) {
	...
	process_tag_test(num_iterations);
